...
const char* ImageMask::COMPONENT_NAME = "ImageMask";

const static int BUFFER_SIZE = 1024;
static char* BUFFER = new char[BUFFER_SIZE];

ImageMask::ImageMask(Object* a_object, AnimControl* a_animControl) : 
    ImageRenderer(a_object, a_animControl)
{
...
void ImageMask::Draw(bool a_preview, double a_delta, Camera* a_camera)
{
    ...
    ImageDraw(a_preview, a_camera);

    renderTexture->Unbind();
}

void ImageMask::Update(double a_delta, Camera* a_camera)
{
    Draw(false, a_delta, a_camera);
}
void ImageMask::UpdatePreview(double a_delta, Camera* a_camera)
{
    Draw(true, a_delta, a_camera);
}
void ImageMask::UpdateGUI(Workspace* a_workspace)
{
    DataStore* store = DataStore::GetInstance();
...
const char* ImageMask::ComponentName() const
{
    return COMPONENT_NAME;
}

void ImageMask::Load(PropertyFileProperty* a_property, AnimControl* a_animControl)
{
    DataStore* store = DataStore::GetInstance();
...
void ImageMask::Save(PropertyFile* a_propertyFile, PropertyFileProperty* a_parent) const
{
