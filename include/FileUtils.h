...
char* ExtractFileFromArchive(const char* a_fileName, mz_zip_archive& a_archive, bool a_term = false);
#define IFSETTOATTVAL(aCmp, bCmp, val, attVal) if (strcmp(aCmp, bCmp) == 0) { val = attVal; }
#define IFSETTOATTVALCPY(aCmp, bCmp, val, attVal) if (strcmp(aCmp, bCmp) == 0) { val = new char[strlen(attVal) + 1]; strcpy(val, attVal); }
