...
#define IFSETTOATTVALV3(aCmp, bCmp, val, attVal) if (strcmp(aCmp, bCmp) == 0) { char* str = (char*)attVal; str = strchr(str, '{');  char* endStr = strchr(str, ','); val.x = StringToFloat(str, endStr);  str = endStr; endStr = strchr(str + 1, ','); val.y = StringToFloat(str, endStr); str = endStr; endStr = strchr(str + 1, '}'); val.z = StringToFloat(str, endStr); }